...
class Validator {
    ...
    /**
     * Check if event name is unique within section
     */
    public static function isEventNameUniqueInSection($conn, $eventName, $sectionId, $excludeEventId = null) {
        $sql = "SELECT id FROM events WHERE title = ? AND section_id = ?";
        if ($excludeEventId) {
            $sql .= " AND id != ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sii", $eventName, $sectionId, $excludeEventId);
        } else {
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $eventName, $sectionId);
        }
        $stmt->execute();
        $result = $stmt->get_result();
        $exists = $result->num_rows > 0;
    ...
    /**
     * Check if all registered students have marks entered for an event
     */
    public static function allStudentsHaveMarks($conn, $eventId) {
        ...
        $stmt2 = $conn->prepare("SELECT COUNT(*) as graded FROM event_results WHERE event_id = ? AND marks IS NOT NULL");
        $stmt2->bind_param("i", $eventId);
        $stmt2->execute();
        $graded = $stmt2->get_result()->fetch_assoc()['graded'] ?? 0;
        $stmt2->close();
     ...
     */
    public static function isStudentRegisteredForEvent($conn, $studentId, $eventId) {
        $stmt = $conn->prepare("SELECT id FROM event_registrations WHERE student_id = ? AND event_id = ?");
