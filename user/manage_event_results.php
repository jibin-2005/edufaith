...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['publish_results'])) {
    ...
    if ($verify_result->num_rows === 0) {
        ...
        $check_published = $conn->prepare("SELECT is_results_published FROM events WHERE id = ?");
        $check_published->bind_param("i", $event_id);
        $check_published->execute();
        $published_check = $check_published->get_result()->fetch_assoc();
        $check_published->close();
