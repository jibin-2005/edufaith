...
// Parents linked to this teacher's classes
$parents = [];
$parent_stmt = $conn->prepare("SELECT DISTINCT p.id, p.username
                               FROM users p
...
$parent_stmt->bind_param("i", $teacher_id);
$parent_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    if (!$send_error) {
        $check = $conn->prepare("SELECT 1
                                 ...
                                 JOIN users s ON ps.student_id = s.id
                                 JOIN classes c ON s.class_id = c.id
                                 WHERE c.teacher_id = ? AND p.id = ? AND p.role = 'parent'");
        $check->bind_param("ii", $teacher_id, $recipient_id);
        $check->execute();
    ...
    if (!$send_error && $recipient_id > 0) {
        ...
        $stmt->bind_param("iiss", $teacher_id, $recipient_id, $subject, $body);
        if ($stmt->execute()) {
...
<body>
    <div class="sidebar">
        ...
        <ul class="menu">
            ...
            <li><a href="messages_teacher.php" class="active"><i class="fa-solid fa-envelope"></i> Messages</a></li>
